...
0	
"""
k=int(input("number of monoster :"))
inpower=int(input("initial power:"))
m=[]
for i in range(k):
    power=int(input(f"the power of monoster {i+1}:"))
    bonous=int(input(f"the bonous for {i+1}:"))
    m.append((power,bonous))#creation of tuple 
m.sort()
print(m)
#k=inpower
d=0
for power,bonous in m:
    if inpower>power:
        inpower+=bonous 
        d+=1 
    else:
